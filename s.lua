...
-- KEY CHECK
-- ============================================================
if getgenv().ASIX_KEY ~= "сынпиздыданечка" then
...
local function createPlatformZ()
    ...
    platformPart.CFrame = CFrame.new(
        hrp.Position + Vector3.new(0, Values.PlatformHeightOffset, 0)
    )
...
local PlatformPanel = Create("Frame", {
    ...
    Size = UDim2.new(0, SIDE_TAB_W, 0, SIDE_TAB_H),
    Visible = false,
    ClipsDescendants = true,
    ZIndex = 5,
    Parent = MainContainer
...
local PlatformScrollLayout = PlatformScroll:FindFirstChildOfClass("UIListLayout")
PlatformScrollLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
...
local function CreateSlider(parent, labelText, minVal, maxVal, valueKey, callback, order, isFloat)
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if draggingSlider and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
            ...
            local val
            if isFloat then
                val = math.floor((minVal + (maxVal - minVal) * rel) * 10 + 0.5) / 10
                labelFrame.Text = labelText .. ": " .. string.format("%.1f", val)
            else
                val = math.floor(minVal + (maxVal - minVal) * rel)
